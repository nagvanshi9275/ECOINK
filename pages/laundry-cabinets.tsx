...
export default function LaundryCabinets({ service, testimonials, faqs, seoSettings }: Props) {
...
export const getServerSideProps: GetServerSideProps = async () => {
    // This file is likely mapped to /laundry-cabinets
    // Slug might be 'laundry-cabinets'
    ...
    try {
        ...
        // Fallback or secondary query if slug differs
        if (!service) {
            // Maybe it's singular?
            service = await prisma.service.findUnique({
                where: { slug: 'laundry-cabinet' }
            });
        }
        ...
        if (!service) {
            ...
            // Check both plural and singular in static data
            const staticService = services.find((s: any) => s.id === slug || s.id === 'laundry-cabinet');
            if (staticService) {
                service = {
