            ...
            if (!exists) {
                ...
                const newBlog = await prisma.blogPost.create({
                    data: {
                        ...
                        content: `<h2>${blog.title}</h2><p class="lead" style="font-size: 1.25rem; color: #4b5563; margin-bottom: 2rem;">${blog.excerpt}</p><hr/><p>This content was automatically migrated from the static site. Administrators can now edit this full content in the CMS.</p><h3>Introduction</h3><p>We believe that <strong>${blog.category}</strong> design requires attention to detail. In this article, we explore the key aspects of <strong>${blog.title}</strong> and how Magri Cabinets can help you achieve your dream home.</p><ul><li>Bespoke Design</li><li>Quality Materials</li><li>Expert Craftsmanship</li></ul><p>Contact us today to learn more.</p>`,
                        category: {
                            connectOrCreate: {
                ...
                results.push({ ...newBlog, status: 'Created' });
            } else {
                results.push({ slug: blog.id, status: 'Skipped (Exists)' });
            }
        }
